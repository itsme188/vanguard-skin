...
import { sendDigestEmail, type DigestMode } from "@/lib/digest/send-digest";
import {
  checkCloudMarker,
...
export async function POST(request: Request) {
  return withCronAuth(request, async () => {
    ...
    if (isMarketHoliday(etToday)) {
      ...
      return { success: true, skipped: true, reason: "market_holiday", date: etToday };
    }
    ...
    try {
      ...
      return await withRunningMarker("digest", async ({ confirmSent }) => {
      ...
      // A thrown DigestSendError propagates through the finally below to
      // withCronAuth's own catch, which duck-types {status, message} and
      // maps it to err.status directly. Generic errors land on the 500 path.
    } finally {
