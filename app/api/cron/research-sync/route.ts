...
import { acquireResearchSyncLock, releaseResearchSyncLock } from "@/lib/research/sync-lock";
...
export async function POST(request: Request) {
  return withCronAuth(request, async () => {
    ...
    // Refuse to start a second overlapping pass — the manual "Sync Feeds"
    // button and the background refresh both POST /api/research/sync, and
    // the AI stages here are select-then-spend. See
    // lib/research/sync-lock.ts for the full rationale.
    const lock = acquireResearchSyncLock("cron");
    if (!lock.ok) {
      return { success: true, skipped: "already_running", heldBy: lock.heldBy };
    }
