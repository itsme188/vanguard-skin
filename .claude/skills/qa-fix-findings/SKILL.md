...
6. **All code work happens in the worktree** (`../vanguard-skin-qa-fix`); the main checkout is used only for cherry-picks, merges, push (PR branch only — never `git push origin main`), and deploy.
...
2. **Anti-strand guard:** run `git branch --list 'qa-auto-fixes-*' --list 'qa-deep-fixes-*'` and both `gh pr list --state open --search "qa-auto-fixes"` and `gh pr list --state open --search "qa-deep-fixes"` from the main checkout. For every unmerged branch/PR: record it for the summary, and mark its findings SKIP for tonight (a fix already exists unmerged — re-fixing it forks the work). Identify covered findings by the `[qa:<finding-id>]` trailer in the branch's commit messages (`git log main..<branch> --format=%B`).
   **Un-PR'd `qa-deep-fixes-*` branches (2026-09-02 anti-strand extension):** the sweep never pushes (its own Step 4 rule), so its branches stranded night after night with nothing in `gh pr list` to surface them (09-01 and 09-02 both sat unlanded). For every `qa-deep-fixes-*` branch that has commits not on `main` AND no open PR with that branch as head: `git push -u origin <branch>` then `gh pr create --head <branch> --title "QA deep-sweep auto-fixes <date from the branch name>" --body-file /tmp/qa-deep-pr-body.md`, using the Step 4 PR-body template (finding ids from the branch's `[qa:<id>]` trailers or commit subjects, generic descriptions only, no ledger values; note "Tests: suite passed on the branch at commit time per the sweep's Step 4"). Push by branch name — no checkout switch, so the clean-checkout preflight is unaffected. Record each opened PR number in the Step 6 summary and the Pushover. Skip in dry-run.
...
3. **Suite gate per finding:** `ANTHROPIC_API_KEY=test-not-a-real-key npx vitest run` + `npx tsc --noEmit` in the worktree (plus `workers/cron` tests if `workers/` was touched). The dummy key is REQUIRED: `tests/ai/generate.test.ts` needs the env var to merely exist (provider construction check; the network is fully mocked) — it passes on main only because `.env.local` loads there, and the secret-less worktree fails those 3 tests every time, which would read as a red suite and revert a good fix (live-discovered, supervised run #2, 2026-07-26). A dummy value cannot authenticate anywhere. Red suite ⇒ revert the finding's changes (`git checkout -- .` of its files), record attempt, count it.
...
3. Worktree: leave intact on any crash/failure (inspection); on a clean run leave it too (persistent by design) but `git -C "$WT" checkout --detach main` so tomorrow's `checkout -B` starts clean. (`checkout main` without `--detach` is IMPOSSIBLE in a linked worktree — the primary checkout holds the `main` ref and git refuses with "already used by worktree"; live-discovered, supervised run #2, 2026-07-26.)
