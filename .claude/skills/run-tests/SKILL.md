...
1. If the user provided a filter argument, run: `npx vitest run tests/{argument}/ --exclude '.claude/**'`
2. If no argument, run all tests: `npx vitest run --exclude '.claude/**'`
...
- Always use `--exclude '.claude/**'` to avoid picking up worktree duplicates.
