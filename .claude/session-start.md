...
- `npm run inbox` FIRST — who is waiting on whom (`docs/reference/coordination.md` § Who acts next): the WAITING ON YOU block (open `decision` records first, then open PRs, tasks, stale ownership) is the first thing the user sees in the synthesized menu, verbatim, before any candidates.
