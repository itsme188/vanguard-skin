 ...
 * quoted in SHARE dollars ("trimmed at $388/share") that lands on an OCC
 * OPTION row is therefore compared against the option PREMIUM, so it can only
 * fire if the premium reaches $388 — and the deliberate option exemption from
 * the plausibility band (lib/levels/scan-range.ts) hides the absurdity instead
 * of flagging it. Live example that produced this module: a GOOGL
 * "trimmed at $388" exit sat armed on the GOOGL Jan-2027 $220 call.
 *
 ...
 * "GOOGL 270115C00220000" entered the list Claude was asked to quote levels
 * for. Newsletters never quote option premiums by OCC symbol; when an author
 * says "GOOGL 388" they mean the share.
 *
 ...
 *      so a held GOOGL contract folds into a held GOOG share row), and DROPS
 ...
 *
 * Real data this has to survive: the user's DB carries BOTH a bare "GOOGL"
 * stub with no type and no price AND the real, held, priced "GOOG" Class C
 * row. Attaching a level to the stub would be technically "not an option" and
 * still useless — the scanner needs a price. Ranking, highest first:
 *
 ...
 *      underlying or an issuer sibling of it (held GOOGL call -> held GOOG);
 ...
 * issuer-family aware: the prompt lists GOOG but a newsletter (and therefore
 * the model) may well say GOOGL, and the two are the same issuer. Exact
 ...
 *               $388-share-level-on-a-$126-premium case.
