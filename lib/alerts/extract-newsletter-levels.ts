 ...
 * (a newsletter's "$52" is a share price), so the underlying is pulled into
 ...
 * put a share-priced exit on a held call of a dual-class issuer, where it was
 * compared against the option premium and could never fire (and the
 * deliberate option exemption from the plausibility band hid that). Every
 * option row is therefore folded into its underlying equity — issuer-family
 * aware, so a held FOXA contract folds into a held FOX row — and an option
 * whose underlying has no equity row anywhere is dropped with a warning. See
 * lib/alerts/option-level-resolution.ts.
 */
...
export async function extractLevelsFromArticle(
  ...
  // Issuer-family aware: the prompt lists FOX but the newsletter (and so the
  // model) may say FOXA — same issuer, same level.
