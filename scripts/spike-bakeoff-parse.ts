...
// spike-only helpers
// ---------------------------------------------------------------------------
// repA/repB (the deterministic HTML->text representations) have been ported,
// behavior-identical, to lib/print-watch/representations.ts (task-3,
// 2026-08-20) — imported above as htmlToTablesRepresentation/htmlToRawText.
// Only this parser-response-defensive helper stays spike-local: it is
// unrelated to representation building and only used below in
// candidatesFromText.
...
/**
 * repA's structured NormalizedTable[] stayed internal when the representation
 * builders were ported to lib/print-watch/representations.ts (task-3,
 * 2026-08-20) — only the rendered text crosses that boundary now, per the
 * task-3 brief's public surface. These run-summary diagnostics are
 * informational only (never read back by any script or test), so they are
 * recovered here from the rendered text instead: the header's "tables kept"
 * line, the body-section length, and each TABLE block's own
 * "spans: colspan=N rowspan=N" / "nested-tables-flattened=N" annotations.
 * empty_columns_pruned / empty_rows_pruned are NOT recoverable this way (the
 * render never prints prune counts) and are dropped rather than faked.
 */
function deriveRepADiagnostics(text: string): Record<string, number> {
  const keptMatch = text.match(/^# tables kept: (\d+)(?: \((\d+) layout\/spacer tables skipped\))?/m);
  const tablesKept = keptMatch ? Number(keptMatch[1]) : 0;
  const layoutTablesSkipped = keptMatch && keptMatch[2] ? Number(keptMatch[2]) : 0;

  const bodyMarker = "=== NON-TABLE BODY TEXT (with [TABLE n] position markers) ===\n";
  const bodyAt = text.indexOf(bodyMarker);
  const bodyChars = bodyAt === -1 ? 0 : text.length - (bodyAt + bodyMarker.length);

  let tablesWithColspan = 0;
  let tablesWithRowspan = 0;
  let nestedTablesFlattened = 0;
  for (const header of text.match(/^--- TABLE \d+ \|.*---$/gm) ?? []) {
    const spans = header.match(/spans: colspan=(\d+) rowspan=(\d+)/);
    if (spans) {
      if (Number(spans[1]) > 0) tablesWithColspan++;
      if (Number(spans[2]) > 0) tablesWithRowspan++;
    }
    const nested = header.match(/nested-tables-flattened=(\d+)/);
    if (nested) nestedTablesFlattened += Number(nested[1]);
  }

  return {
    tables_kept: tablesKept,
    layout_tables_skipped: layoutTablesSkipped,
    body_chars: bodyChars,
    tables_with_colspan: tablesWithColspan,
    tables_with_rowspan: tablesWithRowspan,
    nested_tables_flattened: nestedTablesFlattened,
  };
}
...
function buildInput(
  ...
  if (repr === "A") {
    const builtText = htmlToTablesRepresentation(html);
    writeFileSync(join(dir.path, "parse-input-repA.txt"), builtText, "utf-8");
    const capped = capText(builtText, CLI.maxInputChars);
    return {
      input: {
        symbol: dir.symbol,
        eventDate: dir.eventDate,
        ...
        truncated: capped.truncated,
        originalChars: builtText.length,
        diagnostics: deriveRepADiagnostics(builtText),
      },
    };
  }
...
/**
 * Only run when this file IS the entry script. loadContracts is exported for
 * the scoring pass and for ad-hoc inspection (htmlToTablesRepresentation /
 * htmlToRawText live in lib/print-watch/representations.ts as of task-3,
 * 2026-08-20) — importing this module must never fire a corpus-wide,
 * billable run. (It did once, during development, which is why this guard
 * exists.)
 */
