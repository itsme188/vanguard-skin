...
- **Needs the user's ruling — slice B, Codex finding 16 residual:** a Dow Jones stitched text and an EDGAR exhibit of the same release remain two independent documents in the reconciler and can green together. That is v1's measured pair and the basis for the spec's single-source callouts; the alternative is reconciler "correlation groups" later. B's plan keeps v1 behaviour and asks for the ruling.
