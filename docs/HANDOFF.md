...
**Waiting on:** USER: (a) the tax-lot data steps, now in a fixed order — run the IBKR direction backfill (copy, then live) BEFORE the v2 recompute; the located statement manifest and the expected match/refusal counts are in the gitignored `docs/private/recompute-diagnosis-2026-09-15.md`; (b) five data repairs listed there, each its own decision (CRWD option re-symbol on the Vanguard side, FB→META / TWTR identity merge, UBER in-kind quantity vs donations, opening lots for pre-ledger Vanguard positions, forex legs deferred); (c) the option-attached-levels repair `--apply` (dry run classified 4 move / 1 leave / 7 review / 3 duplicate — the seven `review` rows want a look first). Otherwise nobody: no open PRs, no unlanded qa branches.
