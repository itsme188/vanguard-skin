...
- `docs/superpowers/plans/2026-09-02-live-print-v2-slice-b.md` — NEW. Slice B (`.ts` migration support through a static registry, document identity rebuild with an explicit `--live` cutover script, roads + `recordDelivery`, PDF/URL/IR-page acquisition with a pinned-DNS `node:https` fetch and a full SSRF table, registry shim, post-merge integration task). 16 tasks, 5,741 lines. Written by a forked parallel pass of the same session; its Codex round returned REVISE with 18 findings — 16 folded in full, 2 in part (families regex + full-URL road identity; normalised-text identity `text_sha256` instead of reconciler correlation groups). Header mechanics M1–M19.
- `docs/superpowers/plans/2026-09-02-live-print-v2-registries-contract.md` — NEW. The cross-slice registry contract both plans quote, committed at one path as the fork suggested.
...
- **RULED by the user at session end (2026-09-02 late evening): D1 stands (own column, no-shared-file rule kept) and the v1 document pair stands (no correlation groups).** Recorded in spec §2 and `DECISIONS.md`. The two items below are kept for the reasoning.
- **D1 (Codex finding 1), as it was put to the user:** A stores the Finnhub EPS in a new `earnings_bogeys.eps_consensus_vendor` column and leaves `eps_consensus` NULL on the `'finnhub'` row, because `compileContracts` (`lib/print-watch/contracts.ts`, a slice B/F file) fills the adjusted-EPS expected value from the first non-null `eps_consensus` and the spec rules that A and B share no file. Codex wants the spec's `eps_consensus_basis` column plus a basis filter in `compileContracts` — a three-line edit in B's file. Options: (a) keep D1 (default in the plan); (b) relax the no-shared-file rule for exactly that filter and revert to the spec's column.
- **Slice B, Codex finding 16 residual, as it was put to the user:** a Dow Jones stitched text and an EDGAR exhibit of the same release remain two independent documents in the reconciler and can green together. That is v1's measured pair and the basis for the spec's single-source callouts; the alternative is reconciler "correlation groups" later. B's plan keeps v1 behaviour and asks for the ruling.
