...
- [ ] **Tier 2 — Earnings Hub block on `/dashboard/today`**. Lists every earnings event in `calendar_events` for the current week (any source, any symbol — not just held). Held / watchlist / neither chips. "Add ticker" form posting to a new `/api/calendar/events` CRUD route that inserts a `source='manual'` row. "Refresh from Finnhub" button. Per-row preview-sent / recap-sent / pending status chips. ~2-3 hr. Plan ref: same plan file Tier 2.
- [ ] **Tier 2 — `reaction-snapshot.ts` capture event-symbol bars**. Currently captures SPY/QQQ/TLT + sector ETF on 2h post-release; needs to also fetch a 1-min bar for the *event* symbol so the recap email can say "GLW closed +4.2% vs. SPY +0.1%" instead of just SPY. One extra contract resolution in the existing TWS call. ~30 min.
- [ ] **Tier 3 — Earnings cron automation**. `/api/cron/earnings-{preview,recap}` (X-Cron-Secret + KV-marker dedup per `mac-sent-earnings-{phase}-{eventId}-{YYYY-MM-DD}`). Self-gating runner extension in `lib/calendar/enrichment-runner.ts` that finds preview candidates (release_time ∈ [now+105m, now+135m]) and recap candidates (`enriched_at IS NOT NULL` + no marker) — uses the existing 15-min launchd cadence. Held + watchlist filter at the candidate level. ~3-4 hr.
- [ ] **Tier 3 — `earnings_emails` audit log table** (migration 042). Schema: `id, event_id, phase, sent_at, recipient, ai_input_hash, ai_output_md, error`. Local audit log; markers handle cron dedup but a row that survives a Worker rebuild is the auditable history. ~1 hr.
- [ ] **Tier 3 — Worker fallback path** mirrors briefing/digest cloud fallback. `workers/cron/src/fallback-earnings-{preview,recap}.ts`. Lower priority — primary path runs on a 15-min sweep so a single missed minute won't lose an email. ~3-4 hr if/when needed.
- [ ] **Tier 3 — Weekly Finnhub auto-refresh**. Sunday cron alongside existing briefing pulls fresh `/calendar/earnings` for the upcoming week (week-ahead view). Highlights names Finnhub missed so user can manually-add via the Earnings Hub. ~1 hr.
- [ ] **Tier 3 — Settings UI opt-in/opt-out + per-symbol mute**. AppSettings field (`earningsEmailsEnabled`, `mutedEarningsSymbols`); SettingsModal section. ~2 hr.
- [ ] **Earnings preview — pull user notes** (next phase per user 2026-04-27). For each preview/recap, query `notes` table where `security_id` matches any sibling in the family, last 90 days, types `journal` / `earnings` / `trade_thesis`. Cap content to ~1500 chars per note. Render in a "Your prior notes" section of the prompt above the newsletter section so the AI references the user's own thesis. ~30 min. Schema already exists.
- [ ] **Earnings preview — source hierarchy work** (next phase, scope TBD per user). Currently the composer treats all preferred sources equally and falls back to a wider net only when zero preferred articles match. Future: per-source weighting by historical hit-rate (does what the source said pre-earnings actually predict the move?), per-source content-extraction profiles (TMT Breakout's "bogies" sections vs. VK's free-form prose vs. Eliant's options-heavy framing), and a deduplication step when multiple sources cover the same name. Multi-session item; needs design doc before code.
