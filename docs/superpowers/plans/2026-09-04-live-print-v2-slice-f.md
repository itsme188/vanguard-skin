...
**Fixture (SYNTHETIC ids only).** Seed five earnings events `XMPL1`..`XMPL5`, one per weekday of the current Hub week, via the seeder at `.superpowers/sdd/2026-09-04-live-print-v2-slice-f/e2e-seed.sh` (gitignored) — it ET-anchors the run's Monday and inserts each row keyed off it. **Every seeded `calendar_events` row must carry `week_of` set to that Monday.** The Hub's loader (`getEarningsForWeekDeduped`) selects `WHERE week_of = ?`; a row inserted without it never reaches the Hub table even though `Today's releases` can still surface it from other fields, and the symptom reads exactly like the coverage gate over-filtering the fixture — it is not a coverage bug, it is a missing column, and it is worth ruling out first before doubting anything else. Give `XMPL1` (today) a bogey sheet with a revenue consensus and guidance text, arm its worksheet, and set its `release_time` to **`16:05`, INSIDE the AMC slot (16:00 ET)** — never "a few minutes from now" or any other late-evening clock time: the accept-gate floors on the derived BMO/AMC slot, and a `release_time` at or after 17:00 is treated as a suspect call time, so the watch window renders CLOSED and no live-window step in this runbook can run against it. A synthetic press release lives under the gitignored `data/private/e2e/`.
...
11. **Hidden-tab abort.** Switch to another tab — if real OS-level backgrounding does not flip `document.visibilityState` in this harness, forge it (`Object.defineProperty` on `visibilityState`/`hidden` plus a dispatched `visibilitychange`, which is exactly what `EarningsHubLive.tsx`'s listener reads) — for **49 s, not 30**: the measured cadences are 30 s and 60 s, so a 30 s hidden window can land entirely inside a natural gap between polls and produce a zero that means nothing, while 49 s guarantees the next scheduled tick of every stream falls inside it. The network panel shows no `/api/print-watch/status` requests while hidden and exactly one immediately on return.
