...
-- v1 cardinality: exactly ONE flow-carrying leg per donation (Codex R2-12).
...
- **Recovery id-remap** (Codex R2-6): `recovery.ts` re-inserts transactions with NEW row ids, which would dangle the id-based link/assignment references. Recovery restores transactions first, then remaps `donation_leg_links.transaction_id` and `donation_lots.acquisition_transaction_id` through the transactions' stable `source_key`s; a dedicated recovery test covers the round-trip. The recovery manifest's table enumeration is extended to the three new tables (donations before links/assignments in restore order).
