...
- **Boot:** run the **deployed Electron standalone server** (`/Applications/Vanguard Dashboard.app/Contents/Resources/standalone/server.js`) with `env -i` and an explicit allowlist: `PORT=3097 HOSTNAME=127.0.0.1 NODE_ENV=production DATABASE_PATH=<abs path to copy>` plus `ANTHROPIC_API_KEY`/`FINNHUB_API_KEY`/`FRED_API_KEY` read from `.env.local`. Outbound keys (Resend, Pushover, Gmail, Worker marker URL, cron secret) are simply never passed — every outbound surface already no-ops gracefully on missing keys, so the sandbox cannot send real mail, push, or Worker traffic. `lib/db.ts` honors `DATABASE_PATH` (db.ts:11). Why standalone instead of `npx next dev -p 3097`: (a) two `next dev` processes against one project dir corrupt the Turbopack single-writer cache (safety rule), and (b) the deployed build is what the owner actually uses daily — higher-fidelity QA target. Trade-off: the sandbox lags repo `main` until the next DMG rebuild; FINDINGS.md notes this.
