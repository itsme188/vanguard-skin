...
**Consumer matrix, rebuilt from the call sites.** `coveredForEvent(db, symbol, eventId)` = held or watchlist (family-aware, as today) or `isEventArmed(eventId)`. *(Ruling R11, 2026-09-03: the armed leg is CLUSTER-aware — an event is armed-covered when it, or any unsuperseded earnings twin sharing its `(UPPER(symbol), event_date)`, carries a flag, because the sweep and the cockpit can dedupe to different twins of one print. `isEventArmed` / `getArmedEventIds` keep their exact per-event meaning. See DECISIONS.md.)*
...
- A sender (the sweep tick, plus an immediate post-commit attempt) drains unsent rows in generation order and writes KV key `armed-events` = `{ generation, entries }` only after reading the current KV value and confirming its generation is lower; an out-of-order completion therefore never regresses the key. The row is marked `sent_at` on success; failures retry on the next tick. *(D2: the Mac never writes KV — it POSTs to `/internal/armed-events` and the Worker does the generation compare; a read-only GET twin reports the generation that stands. See DECISIONS.md.)*
...
- **088 (A)**: `earnings_prepare_steps`, `earnings_bogey_scans`, `cloud_outbox`; `earnings_bogeys` rebuild widening `source` with `'finnhub'` and adding `eps_consensus_basis`, `extra_metrics_json` (carried early so the table is rebuilt once; unused until F); snapshot v11. *(Plan deviation D1, 2026-09-03: the column shipped is `eps_consensus_vendor`, not `eps_consensus_basis` — see DECISIONS.md.)*
...
- A prepare step hangs (a stalled model call or TWS socket) → the runner races every invocation against a four-minute deadline strictly inside the five-minute stale-claim window, aborts the step's `AbortSignal`, and books the row `failed`; the sweep tick that awaits the pass is never wedged. Step authors check `ctx.signal.aborted` between units of work and keep side effects idempotent. *(Ruling R13, 2026-09-03 — `PrepareStepContext` gained `signal` additively; see DECISIONS.md.)*
