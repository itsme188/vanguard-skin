...
- **Amount sign** (post-2026-04-01 statement-import era): `BUY`/`BUY_TO_OPEN`/`BUY_TO_CLOSE`/`BUY_TO_COVER` amounts must be negative (cash out); `SELL`/`SELL_TO_CLOSE`/`SELL_TO_OPEN` amounts must be positive (cash in). The parser auto-normalizes a wrong-sign amount on those 7 types (for `trade_date >= 2026-04-01`) to the correct sign with a warning — the fix is applied *before* the row's `source_key` is derived, so a later corrected re-transcription dedups to the same key instead of importing as a duplicate. Still emit the correct sign in the first place. Pre-2026-04 rows are legacy-positive by design and are never touched by this normalization.
...
- **Symbol conventions** (consistency across statements is critical — each variant creates a duplicate `securities` row): for bonds incl. US Treasuries, use the **9-character CUSIP** as the symbol (e.g., `912810SA7`, never the descriptive name like `U S TREASURY BOND 3 2/15/48`). For dual-class equities, use the **slash form** (e.g., `BRK/B`, `LBRD/A`, `LSXM/K`), never `BRK B`, `BRK.B`, or `BRKB`. Each variant becomes a distinct security row in the database — the Vanguard Taxable audit on 2026-05-11 found 3 duplicate pairs caused by mid-year convention drift (`BRK B` → `BRK/B`, two Treasury-name → CUSIP swaps).
...
Vanguard Taxable,2025-07-10,,TRANSFER_IN,XMTR,Xometry Inc Cl A,Stock,100,,2500.00,,Shares journaled in
Vanguard Taxable,2025-07-10,,TRANSFER_OUT,XMTR,Xometry Inc Cl A,Stock,100,,2500.00,,Gifted shares (journal out)
