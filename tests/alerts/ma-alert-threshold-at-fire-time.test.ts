...
import { buildSuggestionContext, buildSuggestionPrompt } from "@/lib/alerts/generate-suggestion";
...
describe("buildSuggestionContext — the AI sentence quotes the real threshold", () => {
  it("uses the recorded fire-time threshold when there is one", () => {
    ...
    expect(ctx?.levelPriceBasis).toBe("recorded");
    expect(buildSuggestionPrompt(ctx!)).toContain("at the time of the cross");
  });
  ...
  it("falls back to the live MA for a pre-093 alert, never to the creation snapshot", () => {
    ...
    expect(ctx?.levelPriceBasis).toBe("current");
    const prompt = buildSuggestionPrompt(ctx!);
    expect(prompt).toContain("current 9-day SMA; fire-time threshold not recorded");
    expect(prompt).not.toContain("at the time of the cross");
  });
  ...
  it("uses the stored price only when nothing better exists (static level, or an MA with no history)", () => {
    ...
    const ctx = buildSuggestionContext(db, maAlert.alertId as number);
    expect(ctx?.levelPrice).toBeCloseTo(60, 6);
    expect(ctx?.levelPriceBasis).toBe("creation");
    expect(buildSuggestionPrompt(ctx!)).toContain("creation snapshot");
    expect(buildSuggestionPrompt(ctx!)).not.toContain("at the time of the cross");
  });
