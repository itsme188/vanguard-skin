 ...
 * OCC OPTION row.
 *
 * Regression: a "trimmed at $388/share" exit landed on a Jan-2027 $220 call,
 * so it was compared against the option PREMIUM (never reachable) and the
 * deliberate option exemption from the plausibility band hid the absurdity.
 * The fix folds every option in the tracked set into its underlying equity
 * (issuer-family aware) before the prompt is built.
 *
...
describe("option-only holding folds into the underlying equity", () => {
  it("lands the level on the equity id, never on the contract", async () => {
    ...
    const optId = seedOption(db, "ZZZ   270115C00220000", "ZZZ", 126.86);
    hold(db, optId);
    const equityId = seedEquity(db, "ZZZ", { price: 338.86 });
    ...
    expect(tracked[0].current_price).toBe(338.86);

    const article = seedArticle(db, "ZZZ: trimming at 388.");
    const prompt = buildExtractionPrompt(article, tracked);
    // The prompt lists the underlying ticker, not the OCC string.
    expect(prompt).toContain("ZZZ (current $338.86) [held via option]");
    expect(prompt).not.toContain("270115C00220000");

    modelReturns([
      { symbol: "ZZZ", level_type: "exit", price: 388, direction: "bearish", confidence: "high" },
    ]);
...
describe("issuer-sibling fold (dual-class)", () => {
  it("folds a held GOOGL contract into the tracked GOOG share row", async () => {
    const db = makeDb();
    const googId = seedEquity(db, "GOOG", { price: 338.86 });
    hold(db, googId, 10);
    const optId = seedOption(db, "GOOGL 270115C00220000", "GOOGL", 126.86);
    hold(db, optId);

    const tracked = getRelevantSymbols(db);
    expect(tracked).toHaveLength(1);
    expect(tracked[0].symbol).toBe("GOOG");
    expect(tracked[0].security_id).toBe(googId);

    const article = seedArticle(db, "Trimmed GOOGL at 388.");
    // The model echoes the newsletter's spelling (GOOGL) even though the
    // prompt listed GOOG — same issuer, so it must still resolve.
    modelReturns([
      { symbol: "GOOGL", level_type: "exit", price: 388, direction: "bearish", confidence: "high" },
    ]);
    const res = await extractLevelsFromArticle(db, article, tracked);
    ...
    expect(rows[0].security_id).toBe(googId);
  });

  it("prefers a same-class equity over a sibling when both are tracked", () => {
    const db = makeDb();
    const googlId = seedEquity(db, "GOOGL", { price: 340 });
    hold(db, googlId, 10);
    const googId = seedEquity(db, "GOOG", { price: 338 });
    hold(db, googId, 10);
    hold(db, seedOption(db, "GOOGL 270115C00220000", "GOOGL", 126.86));

    const tracked = getRelevantSymbols(db);
    expect(tracked.map((s) => s.symbol).sort()).toEqual(["GOOG", "GOOGL"]);
    const bySymbol = indexTrackedSymbols(tracked);
    expect(bySymbol.get("GOOGL")!.security_id).toBe(googlId);
    expect(bySymbol.get("GOOG")!.security_id).toBe(googId);
  });
});
...
describe("option with no equity row anywhere", () => {
  it("drops the contract, writes no level, and warns once", async () => {
    const db = makeDb();
    const optId = seedOption(db, "QQZ   270115C00220000", "QQZ", 12.5);
    hold(db, optId);
    ...
    try {
      ...
      expect(fold.dropped).toEqual([
        {
          optionSymbol: "QQZ   270115C00220000",
          underlying: "QQZ",
    ...
    const article = seedArticle(db, "QQZ exit at 388.");
    modelReturns([
      { symbol: "QQZ", level_type: "exit", price: 388, direction: "bearish", confidence: "high" },
    ]);
    const guardWarn = vi.spyOn(console, "warn").mockImplementation(() => {});
    try {
      const res = await extractLevelsFromArticle(db, article, [
        {
          symbol: "QQZ   270115C00220000",
          security_id: optId,
          current_price: 12.5,
          relationship: "held",
...
describe("resolveEquityForUnderlying", () => {
  it("prefers a typed, priced sibling over an untyped unpriced stub", () => {
    const db = makeDb();
    // Mirrors the live shape: a bare GOOGL stub with no type and no price
    // alongside the real, priced GOOG row.
    const stubId = seedEquity(db, "GOOGL", { type: null, price: null });
    const realId = seedEquity(db, "GOOG", { price: 338.86 });

    const target = resolveEquityForUnderlying(db, "GOOGL");
    expect(target).not.toBeNull();
    expect(target!.security_id).toBe(realId);
    expect(target!.symbol).toBe("GOOG");
    expect(target!.current_price).toBe(338.86);
    expect(stubId).not.toBe(realId);
  });

  it("returns the exact-symbol row when both candidates are equal quality", () => {
    const db = makeDb();
    seedEquity(db, "GOOG", { price: 338 });
    const googlId = seedEquity(db, "GOOGL", { price: 340 });
    expect(resolveEquityForUnderlying(db, "GOOGL")!.security_id).toBe(googlId);
  });

  it("never returns an option row", () => {
    const db = makeDb();
    seedOption(db, "ZZZ   270115C00220000", "ZZZ", 12);
...
describe("underlyingSymbolOf", () => {
  it("parses the OCC symbol", () => {
    expect(underlyingSymbolOf({ symbol: "GOOGL 270115C00220000" })).toBe("GOOGL");
  });
...
describe("provenance note", () => {
  it("is stable for the same contract + date (idempotent re-runs)", () => {
    const note = repairProvenanceNote("ZZZ   270115C00220000", "2026-09-08");
    expect(note).toBe(
      "re-pointed from ZZZ   270115C00220000 by repair-option-attached-levels on 2026-09-08",
    );
    expect(repairProvenanceNote("ZZZ   270115C00220000", "2026-09-08")).toBe(note);
  });
