...
function seedShareLevelOnOption(): { levelId: number; optId: number; eqId: number } {
  const eqId = seedEquity("ZZZ", 41.2);
  const optId = seedOption("ZZZ   270618C00030000", "ZZZ", 4.15);
  const levelId = seedLevel(optId, { price: 52 });
...
describe("collectOptionAttachedLevels", () => {
  it("returns only levels whose security is an option, with the premium", () => {
    ...
    expect(rows[0].option_price).toBe(4.15);
  });
...
describe("planOptionLevelRepairs", () => {
  ...
  it("reviews (never moves) a contract whose underlying has no security row", () => {
    const optId = seedOption("QQZ   270618C00030000", "QQZ", 12.5);
    const levelId = seedLevel(optId, { price: 52 });
  ...
  it("reports duplicate when the equity already carries the identical level", () => {
    const eqId = seedEquity("ZZZ", 41.2);
    const optId = seedOption("ZZZ   270618C00030000", "ZZZ", 4.15);
    seedLevel(eqId, { price: 52 }); // same type + direction + price
    seedLevel(optId, { price: 52 });
  ...
  it("does not call a different level_type or direction a duplicate", () => {
    const eqId = seedEquity("ZZZ", 41.2);
    const optId = seedOption("ZZZ   270618C00030000", "ZZZ", 4.15);
    seedLevel(eqId, { level_type: "entry", price: 52 });
    seedLevel(eqId, { direction: "bullish", price: 52 });
    seedLevel(optId, { price: 52 });
  ...
  it("marks the SECOND identical mover in one run as a duplicate", () => {
    seedEquity("ZZZ", 41.2);
    const optA = seedOption("ZZZ   270618C00030000", "ZZZ", 4.15);
    const optB = seedOption("ZZZ   280121C00250000", "ZZZ", 130.0);
    seedLevel(optA, { price: 52 });
    seedLevel(optB, { price: 52 });
    ...
    const optId = db
      .prepare(
        `INSERT INTO securities (symbol, security_type, asset_class, multiplier)
         VALUES ('ZZZ   270618C00030000', 'Option', 'equity', 100)`,
      )
      .run().lastInsertRowid as number;
    db.prepare(
      "INSERT INTO prices (security_id, date, close_price, source) VALUES (?, '2026-09-04', 4.15, 'manual')",
    ).run(optId);
    seedLevel(optId, { price: 52 });
  ...
  it("folds a dual-class contract onto its issuer sibling", () => {
    const foxId = seedEquity("FOX", 41.2);
    const optId = seedOption("FOXA  270618C00030000", "FOXA", 4.15);
    seedLevel(optId, { price: 52 });

    const plan = planOptionLevelRepairs(db, { today: TODAY });
    expect(plan[0].verdict).toBe("move");
    expect(plan[0].targetSecurityId).toBe(foxId);
    expect(plan[0].targetSymbol).toBe("FOX");
  });
});
...
describe("applyOptionLevelRepairs", () => {
  ...
  it("preserves an existing notes value", () => {
    const eqId = seedEquity("ZZZ", 41.2);
    const optId = seedOption("ZZZ   270618C00030000", "ZZZ", 4.15);
    const levelId = seedLevel(optId, { price: 52, notes: "author reiterated" });
  ...
  it("does not touch a rejected level's review_status while re-pointing it", () => {
    const eqId = seedEquity("ZZZ", 41.2);
    const optId = seedOption("ZZZ   270618C00030000", "ZZZ", 4.15);
    const levelId = seedLevel(optId, { price: 52, review_status: "rejected" });
  ...
  it("writes nothing for leave / review / duplicate rows", () => {
    ...
    // duplicate
    const zzzEq = seedEquity("ZZZ", 41.2);
    const zzzOpt = seedOption("ZZZ   270618C00030000", "ZZZ", 4.15);
    seedLevel(zzzEq, { price: 52 });
    const dupId = seedLevel(zzzOpt, { price: 52 });
