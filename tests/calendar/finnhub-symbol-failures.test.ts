 ...
 *
 * Repro: 3 of 12 per-symbol calendar fetches came back HTTP 429 ("Too many
 * requests"). The server log got 3 `[finnhub] calendar fetch failed …`
 * lines, but the loop swallowed every one into console.warn and still
 * advanced onProgress, so the Earnings Hub showed "Finnhub 12/12 scanned"
 * and then "Refreshed — 2 new". A slice of the universe was never scanned
 * and nothing said so.
 *
