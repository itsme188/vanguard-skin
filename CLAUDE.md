...
- **Sampling strategy per API call** — Each Claude API call is tuned for its task: (1) Chat uses Opus 4.6 + adaptive thinking (temperature locked at 1.0, `effort` defaults to `high`); (2) PDF extraction uses Opus 4.6 + temperature 0 (no thinking — determinism matters more than reasoning for structured JSON); (3) Factor classification uses Sonnet 4 + temperature 0.2 (no thinking — categorization with training examples doesn't need multi-step reasoning). When thinking is enabled, the API requires temperature=1.0 — you cannot set a lower temperature.
