...
- **Mobile Today view** — `/dashboard/today` server component (mobile-first, single column). Four stacked sections: pending alerts (split triggered-today / older-pending), `NearbyLevelsCard` (armed levels within 5%), full-width chat launcher (`OpenChatButton` → `toggle-mobile-chat` event), IBKR holdings sorted by `|unrealized_gain|` desc with `data_quality` chip. `MobileBottomNav` maps Today to the leftmost slot (sun icon); Overview reachable via hamburger drawer. Architectural decision doc: `docs/plans/2026-04-21-mobile-today-view-decision.md`.
...
- **Canonical CSV examples live in THREE places** that must stay in sync: `app/dashboard/components/CanonicalCsvGuide.tsx` (the Import-tab prompt generator — copy-paste target for Claude Co-Work preprocessing of Vanguard PDFs), `docs/canonical-csv-guide.md` (reference), and `tests/import/canonical-csv.test.ts` (fixtures). A stray comma in the TSX DIVIDEND example pushed the dividend amount into `fees` instead of `amount` and silently zeroed the income card for anyone re-running the prompt; drift went undetected because the test fixture and markdown were correct. When adding or editing a transaction-type example, update all three at once.
