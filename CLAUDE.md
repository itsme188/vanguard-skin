...
- `POST /api/calendar/enrich` — post-release enrichment trigger. Body: `{ eventId?: number }` (omit for window-filter sweep: unenriched rows whose `release_time` falls in `[now-2h, now-5min]`). Accepts `X-Cron-Secret` header for Worker/launchd calls; trusted local UI calls without. Runs `lib/calendar/enrichment-runner.ts::runEnrichment` which orchestrates `fetchActualForEvent` + `captureReactionFromTws` per candidate.
